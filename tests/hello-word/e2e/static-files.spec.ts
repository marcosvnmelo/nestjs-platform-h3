...
describe('Static Files (H3 adapter)', () => {
  ...
  describe('basic static file serving', () => {
    ...
    it('should return 404 for non-existent files', async () => {
      await request(app.getHttpServer()).get('/nonexistent.txt').expect(404);
    });
  });
  ...
  describe('prefix option', () => {
    ...
    it('should not serve files without prefix', async () => {
      await request(app.getHttpServer()).get('/test.txt').expect(404);
    });

    it('should serve nested files with prefix', async () => {
      await request(app.getHttpServer())
  ...
  describe('dotfiles option', () => {
    it('should ignore dotfiles by default', async () => {
      ...
      await request(app.getHttpServer()).get('/.hidden').expect(404);
    });
    ...
    it('should deny dotfiles when dotfiles=deny', async () => {
      ...
      await request(app.getHttpServer()).get('/.hidden').expect(403);
    });
  ...
  describe('index option', () => {
    ...
    it('should not serve index when index=false', async () => {
      ...
      await request(app.getHttpServer()).get('/sub/').expect(404);
    });
  });
