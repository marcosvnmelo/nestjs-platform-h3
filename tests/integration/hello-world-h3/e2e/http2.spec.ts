...
describe('HTTP/2 Support (H3 adapter)', () => {
  ...
  describe('HTTP/2 Configuration', () => {
    it('should report isHttp2Enabled as false by default', async () => {
      ...
      expect(adapter.isHttp2Enabled()).toBe(false);
    });
    ...
    it('should create HTTP/1.1 server by default', async () => {
      ...
      expect(server.constructor.name).not.toContain('Http2');
    });
  });

  describe('HTTP/2 Cleartext (h2c)', function (this: any) {
    ...
    it('should initialize h2c server when http2Options is set without TLS', async () => {
      ...
      expect((app.getHttpAdapter() as H3Adapter).isHttp2Enabled()).toBe(true);
    });

    it('should handle requests over h2c', async () => {
      const moduleRef = await Test.createTestingModule({
        controllers: [TestController],
      }).compile();
      ...
      const response = await new Promise<{
        ...
        req.on('response', (httpHeaders) => {
          headers = httpHeaders;
          status = httpHeaders[':status'] as number;
        });
  ...
  describe('HTTP/2 Secure (h2)', function (this: any) {
    ...
    it('should initialize h2 server when http2Options and httpsOptions are set', async function (this: any) {
      if (!certs) {
        this.skip();
        return;
      }
    ...
    it('should handle HTTP/2 requests over TLS', async function (this: any) {
      if (!certs) {
        this.skip();
        return;
      }
    ...
    it('should allow HTTP/1.1 fallback when allowHTTP1 is true', async function (this: any) {
      if (!certs) {
        this.skip();
        return;
      }
