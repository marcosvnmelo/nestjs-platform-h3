...
export class LazyController {
  ...
  @Get('transient')
  async exec() {
    const { TransientLazyModule } = await import('./transient.module.ts');
    const moduleRef = await this.lazyLoadModule.load(() => TransientLazyModule);

    const { TransientService } = await import('./transient.service.ts');
  ...
  async execRequestScope() {
    const { RequestLazyModule } = await import('./request.module.ts');
    const moduleRef = await this.lazyLoadModule.load(() => RequestLazyModule);

    const { RequestService } = await import('./request.service.ts');
